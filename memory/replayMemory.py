...
class ExperienceReplay(object):
    ...
    def remember(self, current_state, a, r, next_state, game_over):
        ...
        self.input_shape = s.shape[1:] #store the shape of the input states for later use
    ...
    def get_batch(self, model, batch_size, gamma=0.9):
        ...
        q_values = np.max(predictions[batch_size:], axis=1).repeat(num_actions).reshape((batch_size, num_actions))
        delta = np.zeros((batch_size, num_actions))
        actions = np.cast['int'](actions)
        delta[np.arange(batch_size), a] = 1
        targets = (1 - delta) * predictions[:batch_size] + delta * (rewards + gamma * (1 - game_overs) * q_values)
